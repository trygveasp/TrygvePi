...
def get_measurement(location):
    ...
    try:
        ...
        if debug: print(weatherData.stationByName(name))
        ...
        if debug_update and test_fail_netatmo:
            print("test_fail_netatmo")
            raise
...
def getLocationForecast(location,vars):
    ...
    if debug: print(request_string)
    try:
        try:
            response = requests.get(request_string, timeout=timeout_forecast)
        except:
            print(request_string+" timed out with timeout "+str(timeout_forecast))
            raise
        ...
        for i, time in enumerate(product.findall('time')):
            ...
            # Not accumulated variables
                ...
                print("Should not happen "+str(dt_from)+str(dt_to))
                raise

        if debug_update and test_fail_forecast:
            print("test_fail_forecast")
            raise
        return accVars1h,otherVars,accVars2h,accVars3h,accVars6h
    except:
        print("Could not get location forecast")
        raise
...
def get_rain(location):
    ...
    try:
        ...
        try:
            ...
            print(request_string + " timed out with timeout " + str(timeout_nowcast))
            raise
        ...
        if debug_update and test_fail_nowcast:
            print("test_fail_nowcast")
            raise
...
class Screen(Frame):
    ...
    def toggle_geom(self,event):
        ...
        if debug: print(geom,self._geom)
        self.master.geometry(self._geom)
        self._geom=geom

    def findXCenter(self, canvas, item):
        if debug: print(self.master.winfo_screenwidth() - self.pad)
    ...
    def getTimeIndices(self,dts,valuesIn=None):
        if valuesIn != None and len(dts) != len(valuesIn):
            raise Exception("Mismatch in length: "+str(len(dts))+" != "+str(len(valuesIn)))
    ...
    def update(self):
        ...
        if debug_update: print(datetime.now())
        self.tick()
        self.days=5
        now=datetime.utcnow()
        ...
        try:
            ...
            self.outdoorTemp=self.netatmoOutdoor.create_text(235, 75, text=tempText(temp), fill=tempColor(temp), font=('verdana', 60))
            ...
            if debug_update: print("Updated netatmo")
        except:
            pass
        ...
        try:
            ...
            if debug_update: print("Updated nowcast")
        except:
            pass
        ...
        try:
            ...
            for time in sorted(accVars1h):
                if debug: print(time,var,accVars1h[time])
                times.append(time)
            ...
            if debug_update: print("Updated forecast")
        except:
            pass

        #if hasattr(self, 'bottomUpdated'): self.netatmo.delete(self.bottomUpdated)
        #self.bottomUpdated = self.bottomFig.create_text(300, 10, text=updated, font=('verdana', 10))
        self.bottomPlots.draw_idle()
        if debug_update:
            global test_fail_netatmo
            test_fail_netatmo = False
            global test_fail_nowcast
            test_fail_nowcast= False
            global test_fail_forecast
            test_fail_forecast = False
        self.after(updtime, self.update) # ask the mainloop to call this method again in ms
...
if __name__ == "__main__":
    ...
    names=["Aspelien Konnerud","Aspelien","Solbakken"]
